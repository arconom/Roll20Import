//todo DR on hit locations
//todo spell difficulty
//todo spell cost
...
var sheetTypeOneController = (function () {
    var selectors = {
        ...
        notes: 'textarea[name="attr_notes"]',
...
var sheetTypeTwoController = (function () {
    ...
    function powers(context, char) {
        ...
        addRows(selectors.spells, context.querySelector(selectors.powersContext), enterSpell, char.spells);
    }
    function skills(context, char) {
        console.log("skills", 'context', context);
        context.querySelector(selectors.skillsTab).click()
        addRows(selectors.skills, context.querySelector(selectors.skillsContext), enterSkill, char.skills);
    }
...
function htmlDecode(input) {
    console.log("htmlDecode", input);
    if (!isNaN(input)) {
        return input;
    } else if (input.length > 0) {
        return new DOMParser().parseFromString(input.replace("&#13;", " "), "text/html").documentElement.textContent;
    } else {
        return "";
    }
}
...
var BlobHandler = (function () {
    ...
    function init() {
        ...
        function _html5WebStorageSave(data, name) {
            ...
            if (blob) {
                name = name || Math.round((new Date()).getTime() / 1000) + ".txt";
                // Produces a unique file name every second.
        ...
        function _startFileRead(fileObject, handler) {
            ...
            reader.abort = this._handleFileReadAbort;
            // "abort" files on abort.
            reader.onerror = this._handleFileReadError;
